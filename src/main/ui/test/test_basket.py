...
def test_add_multiple_cart_items(auth_page):
    catalog = CatalogSteps(auth_page)
    basket = BasketSteps(auth_page)
...
def test_checkout_multiple_items(auth_page):
    ...
    basket.checkout()
    checkout.start_checkout(first_name="Tfkkf",last_name="Krjt",postal_code="123456")
    checkout_total = checkout.get_item_total_after_continue()
    assert checkout_total == basket_total, "Сумма товаров Checkout не совпадает с корзиной"
...
def test_checkout_without_items(auth_page):
    ...
    checkout = CheckoutSteps(auth_page)
    basket.open_cart()
    items = basket.get_items_name()
    assert len(items) == 0, "Корзина не пуста"
    basket.checkout()
    checkout.start_checkout(first_name="Tfkkf",last_name="Krjt",postal_code="")
